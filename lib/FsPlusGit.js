var GitFakeFs = require('gitfakefs'),
    ...
    virtualContentsDirectories = ['HEAD', 'branches', 'tags', 'commits', 'index', 'indexOrWorkingCopy', 'changesInIndex'];
                ...
                if (matchGitRepoInPath) {
                    ...
                    function proxyToGitFakeFs(gitFakeFsConfig) {
                        getGitFakeFs(pathToRepository, gitFakeFsConfig, passError(cb, function (gitFakeFs) {
                            ...
                            // Remove user's cb so we can substitute our own and call cb later:
                            if (typeof args[args.length - 1] === 'function') {
                                args.pop();
                            }
                            args[0] = rootRelativePathInsideGitRepo;
                            args.push(function (err) { // ...
                                if (err && err.name === 'OUTSIDETREE') {
                                    gitFakeFs.getWorkDir(function (getWorkDirErr, workDir) {
                                        var basePath;
                                        if (typeof workDir === 'string') {
                                            basePath = workDir;
                                        } else {
                                            basePath = pathToRepository;
                                        }
                                        args[0] = Path.resolve(basePath, err.relativeTargetPath);
                                        proxyToWrappedFs();
                                    });
                                } else {
                            ...
                            gitFakeFs[fsMethodName].apply(gitFakeFs, args);
                        }));
                    }
                    ...
                    getRepo(pathToRepository, function (err, repo) {
                        if (err) {
                            return cb(fakeFsErrors.Enoent("Error: ENOENT, no such file or directory '" + path + "'"));
                        }
                        if (additionalFragments.length === 0) {
                            ...
                            if (objectType === 'HEAD' || objectType === 'index' || objectType === 'changesInIndex' || objectType === 'indexOrWorkingCopy') {
                                // No further levels

                                return proxyToGitFakeFs({
                                    index: objectType === 'index' || objectType === 'indexOrWorkingCopy',
                                    changesInIndex: objectType === 'changesInIndex',
                                    fallBackToWorkingCopy: objectType === 'indexOrWorkingCopy',
                                    ref: 'HEAD'
                                });
                            }
                            ...
                            if (virtualContentsDirectories.indexOf(objectType) === -1) {
                                ...
                                proxyToGitFakeFs({index: objectType === 'index', ref: objectName});
                            }
                        }
                    });
                } else {
